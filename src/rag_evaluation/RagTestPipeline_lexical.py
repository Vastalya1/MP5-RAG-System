...
the RAG pipeline using BM25 lexical retrieval, storing results in a JSON file.
"""
...
class RAGTestPipeline:
    ...
    async def run_pipeline(
        ...
        if output_path is None:
            ...
            output_path = output_dir / f"rag_results_lexical_{timestamp}.json"
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run the lexical RAG test pipeline on all or selected questions."
    )
    parser.add_argument(
        "--question-numbers",
        type=str,
