...
from orchestration.rag_pipeline import (
...
class QueryDecomposer:
    """
    Plans whether a query should be decomposed and produces up to 3 sub-queries.
    """
...
class SubQuerySynthesizer:
    """
    Synthesizes a final response from multiple independently answered sub-queries.
    """
    ...
    def synthesize(self, query: str, sub_query_results: List[Dict[str, Any]]) -> Dict[str, str]:
        try:
            ...
            response = self.client.chat.complete(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You synthesize grounded insurance-policy answers from sub-query results."},
...
class QueryDecompositionOrchestrator:
    """
    Runs a LangGraph-based decomposition workflow before query rewriting.
    """
    ...
    def _build_graph(self) -> StateGraph:
        ...
        builder.add_conditional_edges(
            ...
            {
                "single": "process_single_query",
                "decompose": "process_sub_queries",
            },
        )
    ...
    def _synthesize_answer_node(self, state: DecompositionState) -> Dict[str, Any]:
        ...
        top_distances = [
            result.get("top_chunk_distance", 2.0)
            for result in sub_query_results
        ]
